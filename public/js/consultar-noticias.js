...
barraDePesquisa.addEventListener("input", (event) => {
  ...
  if (busca !== "") {
    pesquisarNoticiasAdmin(busca, 1);
  } else if (ultimaTecla === "Backspace") {
    capturarNoticiasAdmin(1);
  }
});
...
async function capturarNoticiasAdmin(pagina = 1) {
  try {
    const res = await fetch(`/admin/capturar-noticias?pagina=${pagina}`);
    const data = await res.json();

    if (!res.ok) {
      await exibirAlertaErro("error", "Erro", "Erro ao capturar notícias!");
      throw new Error(data.message || "Erro ao capturar notícias");
    }

    const listaNoticias = document.getElementById("listaNoticias");
    const paginacaoNoticias = document.getElementById("paginacaoNoticias");

    // Se não tiver Nenhuma notícia cadastrado
    if (!data.noticias || data.noticias.length === 0) {
      listaNoticias.style.textAlign = "center";
      listaNoticias.innerHTML = "Nenhuma notícia foi encontrado.";

      if (listaNoticias.classList.contains("active")) {
        listaNoticias.classList.remove("active");
      }
      paginacaoNoticias.innerHTML = "";
    } else {
      listaNoticias.style.textAlign = "left";
      listaNoticias.innerHTML = "";
      paginacaoNoticias.innerHTML = "";
      if (!listaNoticias.classList.contains("active")) {
        listaNoticias.classList.add("active");
      }
      ...
      for (const noticia of data.noticias) {
        ...
        noticiaDiv.appendChild(botaoComentarios);
        noticiaDiv.appendChild(comentarios);
        ...
        const quantidadeDenunciasNoticia = document.createElement("b");
        quantidadeDenunciasNoticia.textContent = await contarDenunciasNoticiaAdmin(noticia.idNoticia);

        denuncias.appendChild(quantidadeDenunciasNoticia);
        if (quantidadeDenunciasNoticia.textContent == 1) {
        ...
        botaoExibirDenuncias.addEventListener("click", async (e) => {
          document.getElementById("listaDenunciasNoticia").innerHTML = "";
          exibirModal(modalDenunciasNoticia, e);
          
          if (quantidadeDenunciasNoticia.textContent == 0) {
            document.getElementById("listaDenunciasNoticia").innerHTML = "<p style='margin-top: 15px; text-align: center;'>Esta notícia não possui nenhuma denúncia.</p>";
          } else {
            paginaDenunciasNoticia = 1;
            await exibirDenunciasNoticiaAdmin(noticia.idNoticia, quantidadeDenunciasNoticiaAprovadas, quantidadeDenunciasNoticiaPendentes, quantidadeDenunciasNoticia);
          }
        });
        ...
        if (quantidadeDenunciasNoticiaAprovadas.textContent < 1) {
          if (noticia.desativado == 0) {
            ...
            noticiaDiv.appendChild(botaoExibirDenuncias);
            noticiaDiv.appendChild(denuncias);
...
async function exibirDenunciasNoticiaAdmin(idNoticia, quantidadeDenunciasNoticiaAprovadas, quantidadeDenunciasNoticiaPendentes, quantidadeDenunciasNoticia) {
  const denunciasNoticia = await capturarDenunciasNoticiaAdmin(idNoticia, paginaDenunciasNoticia);
  
  for (let i = 0; i < denunciasNoticia.denuncias.length; i++) {
    const denunciaNoticia = denunciasNoticia.denuncias[i];

    const denuncia = document.createElement("div");
    denuncia.classList.add("denuncia-noticia");
    ...
    fotoAutorDenuncia.src = denunciaNoticia.fotoPerfil;
    ...
    apelidoAutorDenuncia.textContent = denunciaNoticia.apelido;
    ladoDireitoCabecalho.appendChild(apelidoAutorDenuncia)
  
    const dataDenuncia = document.createElement("p");
    dataDenuncia.textContent = "Data publicação: " + formatarData(denunciaNoticia.dataDenuncia);
    ...
    categoriaDenuncia.textContent = "Categoria: " + denunciaNoticia.categoria;
    ...
    statusD.textContent = denunciaNoticia.status;
    ...
    if (denunciaNoticia.dataRevisao == null) {
      ...
      revisao.textContent = denunciaNoticia.dataRevisao;
      ...
      apelidoAdmin.textContent = denunciaNoticia.apelidoAdmin;
    ...
    descricaoDenuncia.textContent = "Descrição: " + denunciaNoticia.descricao;
    ...
    botaoIgnorarDenuncia.classList.add("ignorar-denuncia-noticia-btn");
    ...
    botaoIgnorarDenuncia.addEventListener("click", async function() {
      ...
      await ignorarDenunciaNoticia(denunciaNoticia.idDenunciaNoticia, denunciaNoticia.idNoticia);
      
      paginaDenunciasNoticia = 1;
      document.getElementById("listaDenunciasNoticia").innerHTML = "";
      quantidadeDenunciasNoticia.textContent = await contarDenunciasNoticiaAdmin(idNoticia);
      if (quantidadeDenunciasNoticia.textContent == 0) {
        document.getElementById("listaDenunciasNoticia").innerHTML = "<p style='margin-top: 15px; text-align: center;'>Esta notícia não possui nenhuma denúncia.</p>";
      } else {
        paginaDenunciasNoticia = 1;
        await exibirDenunciasNoticiaAdmin(idNoticia, quantidadeDenunciasNoticiaAprovadas, quantidadeDenunciasNoticiaPendentes, quantidadeDenunciasNoticia);
      }

      await capturarNoticiasAdmin(1);

      quantidadeDenunciasNoticiaAprovadas.textContent = await contarDenunciasNoticiaAprovadasAdmin(idNoticia);
      if (quantidadeDenunciasNoticiaAprovadas.textContent == 2) {
      ...
      quantidadeDenunciasNoticiaPendentes.textContent = await contarDenunciasNoticiaPendentesAdmin(idNoticia);
      if (quantidadeDenunciasNoticiaPendentes.textContent == 0) {
        quantidadeDenunciasNoticiaPendentes.style.color = "#0F7124";
      } else {
        quantidadeDenunciasNoticiaPendentes.style.color = "#C40303";
    ...
    if (denunciaNoticia.status != "Aprovada") {
      botaoAprovarDenuncia.classList.add("aprovar-denuncia-noticia-btn");
      botaoAprovarDenuncia.textContent = "Aprovar";
      
      botaoAprovarDenuncia.addEventListener("click", async function() {
        botaoAprovarDenuncia.disabled = true;
        botaoIgnorarDenuncia.disabled = true;
        await aprovarDenunciaNoticia(denunciaNoticia.idDenunciaNoticia, apelido, denunciaNoticia.idNoticia);
    ...
    if (i === denunciasNoticia.denuncias.length - 1) {
      const observer = new IntersectionObserver((entries, observerRef) => {
        entries.forEach(async entry => {
          if (entry.isIntersecting && entry.intersectionRatio === 1) {
            ...
            const totalDenunciasNoticia = parseInt(quantidadeDenunciasNoticia.textContent, 10); // Total geral
            const denunciasNoticiaAtuais = denunciasNoticia.denuncias.length; // Comentários na página atual

            if ((paginaDenunciasNoticia * denunciasNoticiaAtuais) < totalDenunciasNoticia) {
              paginaDenunciasNoticia++;
              await exibirDenunciasNoticiaAdmin(apelido, quantidadeDenunciasNoticiaAprovadas, quantidadeDenunciasNoticiaPendentes, quantidadeDenunciasNoticia);
            }
          }
        });
      }, {
    ...
    document.getElementById("listaDenunciasNoticia").appendChild(denuncia);
  }
}

async function capturarDenunciasNoticiaAdmin(idNoticia, paginaDenunciasNoticia = 1) {
  try {
    const res = await fetch(
      `/admin/capturar-denuncias-noticia/${encodeURIComponent(idNoticia)}/${encodeURIComponent(paginaDenunciasNoticia)}`
    );

    if (!res.ok) {
      const errorData = await res.json().catch(() => ({}));
      await exibirAlertaErro("error", "Erro", "Erro ao buscar denúncias da notícia!");
      throw new Error(errorData.message || "Erro ao buscar denúncias da notícia");
    }
    ...
    return {
      ...
      totalDenunciasNoticia: data.totalDenunciasNoticia
    };
  } catch (error) {
    console.error(error);
    await exibirAlertaErro("error", "Erro", error.message);
    return { denuncias: [], totalDenunciasNoticia: 0 };
  }
}

async function aprovarDenunciaNoticia(idDenunciaNoticia, apelidoAdmin, idNoticia) {
  try {
    const res = await fetch('/admin/aprovar-denuncia-noticia', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ idDenunciaNoticia, apelidoAdmin, idNoticia }),
    });

    const data = await res.json();

    if (!res.ok) {
      await exibirAlertaErro("error", "Erro", "Erro ao aprovar denúncia notícia!");
      throw new Error(data.message || "Erro ao aprovar denúncia notícia");
    }
...
async function ignorarDenunciaNoticia(idDenunciaNoticia, idNoticia) {
  try {
    const res = await fetch('/admin/ignorar-denuncia-noticia', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ idDenunciaNoticia, idNoticia }),
    });

    const data = await res.json();

    if (!res.ok) {
      await exibirAlertaErro("error", "Erro", "Erro ao ignorar denúncia notícia!");
      throw new Error(data.message || "Erro ao ignorar denúncia notícia");
    }
...
async function pesquisarNoticiasAdmin(busca, pagina = 1) {
  try {
    const res = await fetch(`/admin/pesquisar-noticias?busca=${encodeURIComponent(busca)}&pagina=${pagina}`);
    const data = await res.json();

    if (!res.ok) {
      await exibirAlertaErro("error", "Erro", "Erro ao pesquisar notícias!");
      throw new Error(data.message || "Erro ao pesquisar notícias");
    }
    ...
    if (!data.noticias || data.noticias.length === 0) {
      ...
      for (const noticia of data.noticias) {
        ...
        if (quantidadeDenunciasNoticiaAprovadas.textContent < 3) {
          if (noticia.desativado == 0) {
            // Desativar notícia
            const botaoDesativarNoticia = document.createElement("button");
            botaoDesativarNoticia.classList.add("desativar-noticia-btn");
    ...
    await exibirAlertaErro("error", "Erro", "Erro ao pesquisar notícia!");
    console.error('Erro na requisição: ' + error.message);
  }
}
...
async function exibirDenunciasComentarioAdmin(idComentario, quantidadeDenunciasComentarioPendentes, quantidadeDenunciasComentario, quantidadeComentarios) {
  ...
  for (let i = 0; i < denunciasComentario.denuncias.length; i++) {
    ...
    botaoAprovarDenuncia.addEventListener("click", async function() {
      ...
      quantidadeComentarios.textContent = await contarComentariosNoticia(denunciaComentario.idNoticia);
      ...
      if (quantidadeComentarios.textContent == 0) {
        ...
        paginaComentarios = 1;
        await exibirComentariosNoticia(denunciaComentario.idNoticia, quantidadeComentarios);
      }
